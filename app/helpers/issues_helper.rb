module IssuesHelper
  ...
  def new_issue_button_a(line: nil, stop: nil)
    line ||= stop&.lines&.first
    link_to(
      content_tag(:div, '', class: 'create-issue-a'),
      report_a_path(line_id: line&.id, stop_id: stop&.id),
    )
  end

  def new_issue_button_b(line: nil, stop: nil)
    line ||= stop&.lines&.first
    link_to(
      content_tag(:div, '', class: 'create-issue-b'),
      report_b_path(line_id: line&.id, stop_id: stop&.id),
    )
  end
end
