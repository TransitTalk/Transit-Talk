...
class PagesController < ApplicationController
  def view_issues
    ...
    if !Rails.env.production?
      @nearby_bus_stops = Stop.includes(:lines, :issues).where(serviced_by: "bus").take(10)
      @nearby_train_stops = Stop.includes(:lines, :issues).where(serviced_by: "metro").take(10)
